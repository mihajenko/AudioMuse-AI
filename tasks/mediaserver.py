...
def delete_automatic_playlists():
    ...
    if delete_function:
        for p in playlists_to_check:
            ...
            if p.get("Name", "").endswith("_automatic") and delete_function(
                playlist_id
            ):
                deleted_count += 1
...
def get_recent_albums(limit):
    ...
    if config.MEDIASERVER_TYPE == "jellyfin":
        return jellyfin_get_recent_albums(limit)
    if config.MEDIASERVER_TYPE == "navidrome":
        return navidrome_get_recent_albums(limit)
    if config.MEDIASERVER_TYPE == "lyrion":
        return lyrion_get_recent_albums(limit)
    if config.MEDIASERVER_TYPE == "mpd":
        return mpd_get_recent_albums(limit)
    return []
...
def get_tracks_from_album(album_id):
    ...
    if config.MEDIASERVER_TYPE == "jellyfin":
        return jellyfin_get_tracks_from_album(album_id)
    if config.MEDIASERVER_TYPE == "navidrome":
        return navidrome_get_tracks_from_album(album_id)
    if config.MEDIASERVER_TYPE == "lyrion":
        return lyrion_get_tracks_from_album(album_id)
    if config.MEDIASERVER_TYPE == "mpd":
        return mpd_get_tracks_from_album(album_id)
    return []
...
def download_track(temp_dir, item):
    ...
    if config.MEDIASERVER_TYPE == "jellyfin":
        return jellyfin_download_track(temp_dir, item)
    if config.MEDIASERVER_TYPE == "navidrome":
        return navidrome_download_track(temp_dir, item)
    if config.MEDIASERVER_TYPE == "lyrion":
        return lyrion_download_track(temp_dir, item)
    if config.MEDIASERVER_TYPE == "mpd":
        return mpd_download_track(temp_dir, item)
    return None
...
def get_all_songs():
    ...
    if config.MEDIASERVER_TYPE == "jellyfin":
        return jellyfin_get_all_songs()
    if config.MEDIASERVER_TYPE == "navidrome":
        return navidrome_get_all_songs()
    if config.MEDIASERVER_TYPE == "lyrion":
        return lyrion_get_all_songs()
    if config.MEDIASERVER_TYPE == "mpd":
        return mpd_get_all_songs()
    return []
...
def get_playlist_by_name(playlist_name):
    ...
    if config.MEDIASERVER_TYPE == "jellyfin":
        return jellyfin_get_playlist_by_name(playlist_name)
    if config.MEDIASERVER_TYPE == "navidrome":
        return navidrome_get_playlist_by_name(playlist_name)
    if config.MEDIASERVER_TYPE == "lyrion":
        return lyrion_get_playlist_by_name(playlist_name)
    if config.MEDIASERVER_TYPE == "mpd":
        return mpd_get_playlist_by_name(playlist_name)
    return None
...
def create_playlist(base_name, item_ids):
    ...
    if config.MEDIASERVER_TYPE == "jellyfin":
        jellyfin_create_playlist(base_name, item_ids)
    elif config.MEDIASERVER_TYPE == "navidrome":
        navidrome_create_playlist(base_name, item_ids)
    elif config.MEDIASERVER_TYPE == "lyrion":
        lyrion_create_playlist(base_name, item_ids)
    elif config.MEDIASERVER_TYPE == "mpd":
        mpd_create_playlist(base_name, item_ids)
